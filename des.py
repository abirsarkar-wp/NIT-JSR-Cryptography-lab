...
[[13,2,8,4,6,15,11,1,10,9,3,14,5,0,12,7],
 ...
 [2,1,14,7,4,13,10,0,6,9,8,15,3,12,5,11]]
]
...
def key_schedule(key):
    key = permute(key,PC1); C,D = key[:28],key[28:]
    keys=[]
    for s in shifts:
        C,D=shift(C,s),shift(D,s)
...
def sbox(b):
    ...
    for i in range(8):
        blk=b[i*6:(i+1)*6]
        r=(blk[0]<<1)|blk[5]; c=(blk[1]<<3)|(blk[2]<<2)|(blk[3]<<1)|blk[4]
        out+=[(S[i][r][c]>>3)&1,(S[i][r][c]>>2)&1,(S[i][r][c]>>1)&1,S[i][r][c]&1]
    return out

def des_round(L,R,K):
    return R, xor(L, permute(sbox(xor(permute(R,E),K)),P))

def des_block(b,key,enc=True):
    b=permute(b,IP); L,R=b[:32],b[32:]
    keys=key_schedule(key)
    if not enc: keys=keys[::-1]
    for k in keys: L,R=des_round(L,R,k)
    return permute(R+L,FP)

def str2bits(s): return [(s>>i)&1 for i in reversed(range(64))]
def bits2str(b): return sum(v<<(63-i) for i,v in enumerate(b))
...
def des_ecb_encrypt(data,key): 
    ...
    for i in range(0,len(data),8):
        out+=des_encrypt_block(int.from_bytes(data[i:i+8],"big"),key).to_bytes(8,"big")
    return out
